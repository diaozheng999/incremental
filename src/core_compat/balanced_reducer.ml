...
let parent_index ~child_index = Int.((child_index - 1) / 2)
...
let get_leaf t i = Option_array.unsafe_get t.data (leaf_index t i)
let to_list t = List.init (length t) ~f:(fun i -> get_leaf t i)
let sexp_of_t sexp_of_a t = to_list t |> [%sexp_of : a option list]
