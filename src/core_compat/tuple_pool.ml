...
let failwiths ?strict ~here message a sexp_of_a =
  Error.(raise (create ?strict ~here message a sexp_of_a))
external phys_equal : 'a -> 'a -> bool = "%equal"
...
let concat l = Base.Base.String.concat ~sep:"" l
...
module Pool = struct
  ...
  *)
  ...
  module Tuple_id : sig
    ...
    let init = 0
    let next t = if t = Int.max_value then 0 else t + 1
    let to_int t = t
  ...
  external unsafe_set_int_assuming_currently_int : 'a Uniform_array.t -> int -> int -> unit = "%array_unsafe_set"

  let unsafe_set_header t ~header_index (header : Header.t) =
    unsafe_set_int_assuming_currently_int t header_index (header :> int)
  ;;
  ...
  let set_metadata (type slots) (t : slots t) metadata =
    Uniform_array.unsafe_set t metadata_index (Obj.repr (metadata : slots Metadata.t))
  ;;

  let create_array (type slots) (metadata : slots Metadata.t) : slots t =
    let t = Uniform_array.unsafe_create_uninitialized ~len:(Metadata.array_length metadata) in
...
module Debug (Pool : S) = struct
  ...
  let eprints message a sexp_of_a = 
    Sexp.to_string_hum ([%sexp_of: string * a] (message, a)) |> Js.log
  ...
  let debug name ts arg sexp_of_arg sexp_of_result f =
    ...
    if !show_messages then eprints (concat [ prefix; name ]) arg sexp_of_arg;
