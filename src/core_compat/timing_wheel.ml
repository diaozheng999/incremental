...
module Level_bits = struct
  ...
  let num_bits_internal t =
    List.fold t ~init:Num_key_bits.zero ~f:Num_key_bits.( + )
  ...
  let create_exn ?(extend_to_max_num_bits = false) ints =
    ...
    if List.exists ints ~f:(fun bits -> bits <= 0) then
      raise_s
        [%message
          "Level_bits.create_exn got nonpositive num bits" ~_:(ints : int list)];
      ...
      raise_s
        [%message
          "Level_bits.create_exn got too many bits"
            ...
            (max_num_bits : int)];
    let ints =
      if extend_to_max_num_bits then
        ints @ List.init (max_num_bits - num_bits) ~f:(Fn.const 1)
      else ints
    in
  ...
  let trim t ~max_num_bits =
    if Num_key_bits.( <= ) (num_bits_internal t) max_num_bits then t
    else
      let rec loop t ~remaining =
        match t with
        | [] -> []
        | b :: t ->
            if Num_key_bits.( >= ) b remaining then [ remaining ]
...
module Alarm_precision : sig
  ...
  let sexp_of_t v = Time_ns.Span.sexp_of_t (to_span v)
  ...
  let interval_num t time =
    Int63.shift_right (time |> Time_ns.to_int63_ns_since_epoch) t
  ...
  let of_span_floor_pow2_ns span =
    if Time_ns.Span.( <= ) span Time_ns.Span.zero then
      raise_s
        [%message
...
module Config = struct
  ...
  let max_num_level_bits alarm_precision =
    Num_key_bits.( - ) Num_key_bits.max_value
  ...
  let invariant t =
    Invariant.invariant [%here] t [%sexp_of: t] (fun () ->
        ...
        Fields.iter ~alarm_precision:ignore ~capacity:ignore
          ~level_bits:(check Level_bits.invariant))

  let create ?capacity ?(level_bits = level_bits_default) ~alarm_precision () =
    let level_bits =
      Level_bits.trim level_bits
        ~max_num_bits:(max_num_level_bits alarm_precision)
    in
    { alarm_precision; level_bits; capacity }

  let microsecond_precision () =
    create () ~alarm_precision:Alarm_precision.about_one_microsecond
      ~level_bits:(Level_bits.create_exn [ 10; 10; 6; 6; 5 ])

  let durations t =
    List.folding_map t.level_bits
      ~init:
        (Alarm_precision.num_key_bits t.alarm_precision |> Num_key_bits.to_int)
      ~f:(fun num_bits_accum level_num_bits ->
        let num_bits_accum =
          num_bits_accum + (level_num_bits |> Num_key_bits.to_int)
        in
        let duration =
          Time_ns.Span.of_int63_ns
            (if num_bits_accum = Int63.num_bits - 1 then Int63.max_value
...
module Priority_queue : sig
  ...
  val internal_add :
    'a t -> key:Key.t -> at:Time_ns.t -> 'a -> 'a Internal_elt.t
  ...
  module Increase_min_allowed_key_result : sig
    type t = Max_allowed_key_did_not_change | Max_allowed_key_maybe_changed
  end
  ...
  module Key : sig
    ...
    val slot :
      t -> bits_per_slot:Num_key_bits.t -> slots_mask:Slots_mask.t -> int
  end = struct
    module Slots_mask = struct
      type t = Int63.t [@@deriving compare, sexp_of]

      let create ~level_bits =
        Int63.( - ) (Num_key_bits.pow2 level_bits) Int63.one

      let next_slot t slot = (slot + 1) land Int63.to_int_exn t
    end

    let num_keys num_bits = Num_key_bits.pow2 num_bits |> Int63.to_float
    ...
    module Span = struct
      ...
      external float_of_int : int -> float = "%identity"

      let to_int63 t = Int63.of_float t

      let of_int63 i = Int63.to_float i

      let scale_int t i = t * float_of_int i

      let pred t = t - 1.

      let succ t = t + 1.

      external to_int_exn : float -> int = "%intoffloat"
    end
    ...
    let add t i = t + of_float i

    let add_clamp_to_max t i =
      if t > max_value - of_float i then max_value else t + of_float i

    let succ_clamp_to_max t = if t = max_value then max_value else succ t

    let sub t i = t - of_float i

    let diff t1 t2 = t1 - t2 |> to_float

    let rem t i = of_float i |> rem t |> to_float
  ...
  module Internal_elt : sig
    ...
    let of_external_exn pool t =
      if is_valid pool t then t
      else raise_s [%message "Timing_wheel got invalid alarm"]
    ...
    let max_alarm_time pool first ~with_key =
      ...
      while !continue do
        let next = next pool !current in
        if Key.equal (key pool !current) with_key then
  ...
  module Level = struct
    ...
    let compute_min_allowed_key t ~prev_level_max_allowed_key =
      ...
      if Key.equal prev_level_max_allowed_key Key.max_value then Key.max_value
  ...
  module Elt = struct
    ...
    let key p t =
      Internal_elt.key p.pool (Internal_elt.of_external_exn p.pool t)

    let value p t =
      Internal_elt.value p.pool (Internal_elt.of_external_exn p.pool t)
  end
  ...
  module Pretty = struct
    module Elt = struct
      type 'a t = { key : Key.t; value : 'a } [@@deriving sexp_of]
    end
  ...
  let compute_diff_max_min_allowed_key ~level_bits ~bits_per_slot =
    ...
    if Num_key_bits.equal bits Num_key_bits.max_value then Key.Span.max_value
  ...
  let invariant invariant_a t : unit =
    ...
    let level_invariant level =
      Invariant.invariant [%here] level [%sexp_of: _ Level.t] (fun () ->
          ...
          Level.Fields.iter
            ...
            ~bits:
              (check (fun bits ->
                   assert (Num_key_bits.( > ) bits Num_key_bits.zero)))
            ~slots_mask:
              (check
            ...
            ~min_key_in_same_slot_mask:
              (check (fun min_key_in_same_slot_mask ->
                   assert (
                     Min_key_in_same_slot_mask.equal min_key_in_same_slot_mask
            ...
            ~diff_max_min_allowed_key:
              (check
                 ([%test_result: Key.Span.t]
                    ~expect:
                     (compute_diff_max_min_allowed_key ~level_bits:level.bits
                         ~bits_per_slot:level.bits_per_slot)))
            ~length:
              (check (fun length ->
                   assert (
            ...
            ~max_allowed_key:
              (check (fun max_allowed_key ->
                   [%test_result: Key.t] max_allowed_key
                     ~expect:
                       (Key.add_clamp_to_max level.min_allowed_key
                          level.diff_max_min_allowed_key)))
            ~slots:
              (check (fun slots ->
                   Array.iter slots ~f:(fun elt ->
                       if not (Internal_elt.is_null elt) then (
                         ...
                         Internal_elt.iter pool elt ~f:(fun elt ->
                             ...
                             assert (
                               Internal_elt.level_index pool elt = level.index);
    ...
    Invariant.invariant [%here] t [%sexp_of: _ t_internal] (fun () ->
        ...
        Fields.iter
          ...
          ~min_elt:
            (check (fun elt_ ->
                 if not (Internal_elt.is_null elt_) then (
                   assert (Internal_elt.is_valid t.pool elt_);
                   assert (
                     Key.equal t.elt_key_lower_bound
                       (Internal_elt.key t.pool elt_)))))
          ~elt_key_lower_bound:
            (check (fun elt_key_lower_bound ->
                 ...
                 if not (Internal_elt.is_null t.min_elt) then
                   assert (
                     Key.equal elt_key_lower_bound
                       (Internal_elt.key t.pool t.min_elt))))
          ~levels:
            (check (fun levels ->
                 ...
                 Array.iteri levels ~f:(fun level_index level ->
                     ...
                     if level_index > 0 then (
                       ...
                       [%test_result: Key.Span.t] (L.keys_per_slot level)
                         ~expect:
                           (Key.Span.succ prev_level.diff_max_min_allowed_key);
                       [%test_result: Key.t] level.min_allowed_key
                         ~expect:
                           (Level.compute_min_allowed_key level
                              ~prev_level_max_allowed_key:
                                prev_level.max_allowed_key))))))

  (** [min_elt_] returns [null] if it can't find the desired element.  We wrap it up
         afterwards to return an [option]. *)
  let min_elt_ t =
    if is_empty t then Internal_elt.null ()
    else if not (Internal_elt.is_null t.min_elt) then t.min_elt
    else
      let pool = t.pool in
      ...
      while !level_index < num_levels do
        ...
        if Key.( > ) (Level.min_allowed_key level) !min_key_already_found then
          (* We don't need to consider any more levels.  Quit the loop. *)
          level_index := num_levels
        else if level.length = 0 then incr level_index
        else
          (* Look in [level]. *)
          let slots = level.slots in
          let slot_min_key =
          ...
          (if not (Internal_elt.is_null first) then
           ...
           while !continue do
             ...
             if Key.( <= ) current_key !min_key_already_found then (
             ...
             if phys_equal next first || !level_index = 0 then continue := false
             else current := next
           done);
  ...
  let add_elt t elt =
    ...
    let first = slots.(slot) in
    if not (Internal_elt.is_null first) then
      Internal_elt.insert_at_end pool first ~to_add:elt
    else (
      slots.(slot) <- elt;
  ...
  let internal_add_elt t elt =
    ...
    if Key.( < ) key t.elt_key_lower_bound then (
      t.min_elt <- elt;
  ...
  let internal_add t ~key ~at value =
    ...
    if Internal_elt.Pool.is_full t.pool then
      t.pool <- Internal_elt.Pool.grow t.pool;
  ...
  (** [remove_or_re_add_elts] visits each element in the circular doubly-linked list
         ...
         element. *)
  let remove_or_re_add_elts t (level : _ Level.t) first ~t_min_allowed_key
      ~handle_removed =
    let pool = t.pool in
    let current = ref first in
    let continue = ref true in
    while !continue do
      ...
      let next = Internal_elt.next pool !current in
      level.length <- level.length - 1;
      if Key.( >= ) (Internal_elt.key pool !current) t_min_allowed_key then
        add_elt t !current
      else (
        t.length <- t.length - 1;
  ...
  let increase_level_min_allowed_key t (level : _ Level.t)
      ~prev_level_max_allowed_key ~t_min_allowed_key ~handle_removed =
    ...
    let level_min_allowed_key =
      Level.min_key_in_same_slot level
        ~key:
          (Key.min desired_min_allowed_key
    ...
    level.max_allowed_key <-
      Key.add_clamp_to_max desired_min_allowed_key
        level.diff_max_min_allowed_key
  ...
  let increase_min_allowed_key t ~key ~handle_removed :
      Increase_min_allowed_key_result.t =
    if Key.( <= ) key (min_allowed_key t) then Max_allowed_key_did_not_change
    else
      ...
      let level_index = ref 0 in
      let result =
        ref Increase_min_allowed_key_result.Max_allowed_key_maybe_changed
      in
      ...
      while !level_index < num_levels do
        ...
        if Key.equal (Level.min_allowed_key level) min_allowed_key_before then (
  ...
  let create ?capacity ?level_bits () =
    let level_bits =
      match level_bits with Some l -> l | None -> Level_bits.default
    in
    let _, _, levels =
      List.foldi level_bits ~init:(Num_key_bits.zero, Key.zero, [])
        ~f:(fun
          ...
          in
          let min_allowed_key =
            Key.min_key_in_same_slot max_level_min_allowed_key
              min_key_in_same_slot_mask
          in
          let max_allowed_key =
    ...
    if level.length > 0 then
      ...
      if not (Internal_elt.is_null !first) then
        ...
        while !continue do
          ...
          if phys_equal next !first then continue := false else current := next;
          if Time_ns.( <= ) (Internal_elt.at pool elt) now then (
...
let sexp_of_t_now _ t = [%sexp_of: Time_ns.t] (t.now)
...
module Pretty = struct
  ...
  module Alarm = struct
    type 'a t = { at : Time_ns.t; value : 'a } [@@deriving fields, sexp_of]
...
let interval_num_start t interval_num =
  if Interval_num.( < ) interval_num min_interval_num then
    raise_interval_num_start_got_too_small interval_num;
  if Interval_num.( > ) interval_num t.max_interval_num then
    raise_interval_num_start_got_too_large t interval_num;
...
let next_alarm_fires_at t =
  ...
  if Internal_elt.is_null elt then None
  else
    let key = Internal_elt.key (pool t) elt in
    if Interval_num.equal key t.max_interval_num then None
    else Some (next_alarm_fires_at_internal t key)

let next_alarm_fires_at_exn t =
  let elt = Priority_queue.min_elt_ t.priority_queue in
  if Internal_elt.is_null elt then
    raise_next_alarm_fires_at_exn_of_empty_timing_wheel t;
  let key = Internal_elt.key (pool t) elt in
  if Interval_num.equal key t.max_interval_num then
    raise_next_alarm_fires_at_with_all_alarms_in_max_interval t;
...
let compute_max_allowed_alarm_time t =
  ...
  if Interval_num.( >= ) max_allowed_key t.max_interval_num then max_time
  else
    Time_ns.add
...
let invariant invariant_a t =
  Invariant.invariant [%here] t [%sexp_of: _ t] (fun () ->
      let check f = Invariant.check_field t f in
      Fields.iter ~config:(check Config.invariant)
        ~start:
          (check (fun start ->
        ...
        ~now:
          (check (fun now ->
               ...
               assert (
                 Interval_num.equal (interval_num t t.now)
        ...
        ~max_allowed_alarm_time:
          (check (fun max_allowed_alarm_time ->
               [%test_result: Time_ns.t] max_allowed_alarm_time
...
let advance_clock t ~to_ ~handle_fired =
  if Time_ns.( > ) to_ (now t) then (
    ...
    | Max_allowed_key_did_not_change ->
        if debug then
          assert (
            Time_ns.( = ) t.max_allowed_alarm_time
              (compute_max_allowed_alarm_time t))
    | Max_allowed_key_maybe_changed ->
        t.max_allowed_alarm_time <- compute_max_allowed_alarm_time t)

let create ~config ~start =
  if Time_ns.( < ) start Time_ns.epoch then
    raise_s
      [%message
        "Timing_wheel.create got start before the epoch" (start : Time_ns.t)];
  let t =
    {
      config;
      start;
      max_interval_num =
        interval_num_internal ~time:max_time
      ...
      priority_queue =
        Priority_queue.create ?capacity:config.capacity
...
let add_at_interval_num t ~at value =
  Internal_elt.to_external
    (Priority_queue.internal_add t.priority_queue ~key:at
       ~at:(interval_num_start t at) value)
...
let ensure_can_schedule_alarm t ~at =
  if Time_ns.( > ) at t.max_allowed_alarm_time then
    raise_that_far_in_the_future t at;
  if Time_ns.( < ) at t.now_interval_num_start then
    raise_before_start_of_current_interval t at
...
let fire_past_alarms t ~handle_fired =
  Priority_queue.fire_past_alarms t.priority_queue ~handle_fired
    ~key:(now_interval_num t) ~now:t.now
